...
class MQTTDTNBrokers:
  ...
  def __init__(self, publish_on_pubrel=True,
    ...
    logger.info("MQTT-DTN Paho Test Broker")
  ...
  def connect(self, sock, packet, callback):
    ...
    resp = MQTTSN.Connacks()
    if me == None:
      me = MQTTDTNClients(packet.ClientId, packet.Flags.CleanSession, packet.Duration, sock, self)
    else:
